...
# Models 
- state: includes position, velocity, & attitude quaternion 
- **State**: The state vector $\mathbf{x} \in \mathbb{R}^{10}$ comprises position, velocity, and the attitude quaternion to avoid gimbal lock:
  ...
  *(Note: $\mathbf{q} = [q_w, q_x, q_y, q_z]^\top$ represents the rotation from the body frame to the inertial frame).*
